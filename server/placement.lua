...
function request_placement(player_id, x, y, z)
    ...
    if not is_reasonable_coordinates(player_id, vector3(x, y, z), 10) then
    ...
    local obj = CreateObject(modelHash, x, y, z - 1, true, false, true)
    local timeout = 0
    ...
    local claymore_id = claymore_create(player_id, netId, x, y, z);

    local permissions = { { player_id = player_id, permission = "pickup" } }

    Entity(obj).state:set('claymore', true, true);
    Entity(obj).state:set('claymore_id', claymore_id, true);
    Entity(obj).state:set('owner', player_id, true);
...
function request_pickup(player_id, claymore_id)
    ...
    if (can_action(player_id, claymore_id, "pickup") == false) then
...
function request_defuse(player_id, claymore_id)
    ...
    -- Check if user has permission to remove this Claymore.
    if (can_action(player_id, claymore_id, "defuse") == false) then
        TriggerClientEvent('ox_lib:notify', player_id, { type = 'error', title = 'Claymore Failed', description = "No permission." });
        return;
    end
